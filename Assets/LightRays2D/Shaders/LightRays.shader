Shader "Custom/LightRays"{
	Properties{
		...
		_Edges("Fade Edges", Range(0.0, 0.2))=0.1
	}
	SubShader{
        Tags{
        	"Queue"="Transparent"
		...
		Pass{
			...
			fixed alphaEdges(fixed pos){

				if (pos < 0.5){
					return clamp(pos/_Edges, 0.0, 1.0);
				}else{
					return clamp((1-pos)/_Edges, 0.0, 1.0);
				}
			}
